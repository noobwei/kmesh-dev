...
PARAMS=()
NAME="kind-kmesh-testing"  # Default cluster name
while (( "$#" )); do
    case "$1" in
    --skip-install-dep)
    ...
    --use-cluster)
      if [ -n "$2" ] && [[ "$2" != --* ]]; then
          ...
          if ! kind get clusters | grep -q "^${NAME}$"; then
              echo "Error: No such cluster '${NAME}'"
              exit 1
          fi
          echo "Using existing cluster '${NAME}'"
          kubectl config use-context "kind-${NAME}"  # Switch kubectl context to the specified cluster
          SKIP_SETUP=true  # Skip the cluster setup since we're using an existing one
          shift
      else
